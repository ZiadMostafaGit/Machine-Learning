...
def plt_contour_multi(x, y, w, b, ax, prange, p1, p2, title="", xlabel="", ylabel=""):
    contours = [1e2, 2e2, 3e2, 4e2, 5e2, 6e2, 7e2,
                8e2, 1e3, 1.25e3, 1.5e3, 1e4, 1e5, 1e6, 1e7]
    px, py = np.meshgrid(np.linspace(
        *(prange[p1])), np.linspace(*(prange[p2])))
    z = np.zeros_like(px)
    n, _ = px.shape
...
def plot_cost_i_w(X, y, hist):
    ...
    cst = [compute_cost(X, y, np.array([wr[i], -32, -67, -1.46]), 221)
...
def load_house_data():
    data = np.loadtxt("./data/student.txt", delimiter=',', skiprows=1)
    X = data[:, :4]
    y = data[:, 4]
    return X, y
